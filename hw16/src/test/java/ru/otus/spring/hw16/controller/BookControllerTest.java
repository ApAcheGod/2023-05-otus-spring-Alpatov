package ru.otus.spring.hw16.controller;
...
import ru.otus.spring.hw16.configuration.SecurityConfiguration;
import ru.otus.spring.hw16.dto.BookDto;
import ru.otus.spring.hw16.dto.CommentDto;
import ru.otus.spring.hw16.entity.Author;
import ru.otus.spring.hw16.entity.Book;
import ru.otus.spring.hw16.entity.Comment;
import ru.otus.spring.hw16.entity.Genre;
import ru.otus.spring.hw16.mapper.*;
...
@Import({BookMapperImpl.class, AuthorMapperImpl.class, CommentMapperImpl.class, SecurityConfiguration.class})
class BookControllerTest {
    ...
    void denyCreateBookForUser() throws Exception {
        mvc.perform(post("/api/book")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isForbidden());
    }
    ...
    @Test
    @WithMockUser(username = "USER", authorities = {"ROLE_USER"})
    void denyUpdateBookForUser() throws Exception {
        mvc.perform(put("/api/book")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isForbidden());
    }
    ...
    @Test
    @WithMockUser(username = "USER", authorities = {"ROLE_USER"})
    void denyDeleteBookForUser() throws Exception {
        mvc.perform(delete("/api/book/" + UUID.randomUUID()))
                .andExpect(status().isForbidden());
    }
